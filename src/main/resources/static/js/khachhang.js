approute.controller("khachhang", ['$scope', '$location', '$http', '$routeParams', function ($scope, $location, $http, $routeParams) {
    $scope.account = [];
    $scope.roles = [];
    $scope.form = {
        username: '',
    ...
    $scope.searchAccounts = function () {
        $http.get(`/rest/account/searchCustomer?keyword=` + $scope.searchQuery) // Thay keyword bằng searchQuery
            .then(resp => {
    ...
    $scope.xuatExcel = function () {
        $http({
            method: 'GET',
            url: `/rest/account/excelCustomer`,
    ...
    $scope.themkhachhang = function () {
        $location.path('/editkhachhang');  // Không có ID, chỉ đến trang thêm tài khoản mới
    };

    // Hàm chuyển hướng đến trang chỉnh sửa với ID
    $scope.suakhachhang = function (id) {
        $location.path(`/editkhachhang/${id}`);  // Điều hướng đến trang chỉnh sửa với ID
    };
    // Khi nhấn nút "Edit", chuyển hướng đến trang chỉnh sửa kèm ID
    $scope.edit = function (item) {
        $scope.suakhachhang(item.id);  // Điều hướng với ID của tài khoản được chọn
    };


    // Load danh sách tài khoản
    $scope.load = function () {
        $http.get(`/rest/account/list?roleIds=3&page=${$scope.currentPage}&size=${$scope.pageSize}`).then(resp => {
    ...
    $scope.saveOrCreate = function () {
        // Đảm bảo role mặc định là Customer (id = 3) nếu chưa được chọn
        if (!$scope.form.role || !$scope.form.role.id) {
            $scope.form.role = { id: 3, name: "Customer" }; // Cung cấp cả id và name cho role
        }
        if ($scope.isNew) {
            // Tạo tài khoản mới
